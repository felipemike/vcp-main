...
require('_inc_/header.php');
	...
	if(empty($_GET['id'])) {
		...
		if($vcp->getTopInfos($id)) {
			...
			$res = $vcp->index[0];
			$vc = (!$vcp->voteTime($res['id'], $res['minutos']) || !$vcp->voteCookie($res['id'], $res['minutos'])) ? 'N�o pode votar' : 'Pode votar';
			...
			echo '<div id="top-infos">
				...
				<strong class="c1">Voc�:</strong> ', $vc, '<br />';
				if(!$vcp->voteCookie($res['id'], $res['minutos']) || !$vcp->voteTime($res['id'], $res['minutos'])) {
					...
					echo '<strong class="c1">Pr�ximo voto em: </strong>', date('d/m/Y H:i:s', $data);
				} else {
					echo '
